...
import { exportResumeFile, postSse } from "./api/sse";
...
const analysisMeta = ref<{ cached?: boolean; fallback?: boolean }>({});
...
const optMeta = ref<{ fallback?: boolean }>({});

const feedbackRefine = ref("");
const refineErr = ref("");
const refining = ref(false);
const refineMeta = ref<{ fallback?: boolean }>({});
...
async function onUpload(e: Event) {
  ...
  try {
    ...
    if (data.warnings?.length) {
      uploadHint.value = data.warnings.join("；");
    }
  } catch {
...
function handleSseError(ev: Record<string, unknown>, fallback: (msg: string) => void) {
  ...
  try {
    await postSse("/api/analyze/stream", { resume: resume.value, jd: jd.value }, (ev) => {
      ...
      if (ev.type === "done" && ev.result && typeof ev.result === "object") {
        ...
        analysisMeta.value = {
          cached: ev.cached === true,
          fallback: ev.fallback === true,
        };
      }
    });
    if (!analysisResult.value && !analysisErr.value) {
      analysisErr.value = "未收到完整分析结果";
    }
  } catch (e) {
...
async function runOptimize() {
  ...
  try {
    ...
    if (!optimized.value && !optErr.value) {
      optErr.value = "未收到优化正文";
    }
  } catch (e) {
...
async function runRefine() {
  ...
  try {
    ...
    if (!optimized.value && !refineErr.value) {
      refineErr.value = "未收到返修正文";
    }
  } catch (e) {
...
async function doExport(fmt: "docx" | "pdf" | "txt" | "md") {
...
<template>
  <div>
    <h1>简历优化 Agent</h1>
    <p class="sub">
      流式分析 / 优化，UTF-8 编码；导出失败时可自动降级为 .txt / .md 本地下载。
    </p>

    <section>
      <h2>1. 简历与岗位</h2>
      <label>简历正文（支持粘贴中英文）</label>
      <textarea v-model="resume" placeholder="在此粘贴简历…" spellcheck="false" />
      <div class="row">
        <label class="inline">
      ...
      <label>职位描述 JD</label>
      <textarea v-model="jd" placeholder="粘贴 JD…" spellcheck="false" />
      <div class="row">
      ...
      <div v-if="analysisResult" class="result-box">
        ...
        <ul>
          <li v-for="(h, i) in analysisResult.highlights" :key="'h' + i">{{ h }}</li>
        </ul>
        <p>主要缺口</p>
        <ul>
          <li v-for="(g, i) in analysisResult.gaps" :key="'g' + i">{{ g }}</li>
        </ul>
        <p>优化建议</p>
        <ul>
          <li v-for="(s, i) in analysisResult.suggestions" :key="'s' + i">{{ s }}</li>
        </ul>
      </div>
      ...
      <template v-if="analysisResult">
        ...
        <textarea v-model="feedbackAnalysis" placeholder="可选：希望侧重哪些方面…" />
        <div class="row">
    ...
    <section v-if="analysisResult">
      <h2>2. 生成优化简历</h2>
      <label class="inline">
        <input v-model="confirmOptimize" type="checkbox" />
      ...
      <div class="row">
        ...
        <button type="button" class="secondary" :disabled="optimizing" @click="resetOptimizeUi">
          清空优化稿
        </button>
      </div>
      ...
      <template v-if="optimized.trim()">
        <h2 style="margin-top: 1rem">3. 导出</h2>
        <div class="row">
          <button
            type="button"
            class="secondary"
        ...
        <h2 style="margin-top: 1rem">4. 对优化稿返修</h2>
        <label>意见（可留空让模型换表述）</label>
        <textarea v-model="feedbackRefine" placeholder="可选：缩短篇幅 / 加强某段 / 更偏技术…" />
        <div class="row">
